...
def login(verbose = False, iq = None, checkConnection = False):
    ...
    if iq == None:
      ...
      iq=IQ_Option('dannyprats@gmail.com','praDos781*') # YOU HAVE TO ADD YOUR USERNAME AND PASSWORD
      iq.connect()
...
def higher(iq,Money,Actives):
    ...
    #for pars in Actives:
    #(done, id) = iq.buy_digital_spot("EURUSD-OTC", 1, "call", 1)
...
def lower(iq,Money,Actives):
    ...
    #(done, id) = iq.buy_digital_spot("EURUSD-OTC", 1, "put", 1)
...
def get_data_needed(iq): #function to gather all the data
    ...
    for active in actives:
        ...
        for candle in current:
            ...
            #useful_frrame_df = pd.DataFrame.from_records([useful_frame])
            #main = pd.concat([main, useful_frrame_df], ignore_index=True, axis=1)  # how to handle index depends on context
...
def get_profit(iq):
    return iq.get_all_profit()['EURUSD-OTC']['turbo']
